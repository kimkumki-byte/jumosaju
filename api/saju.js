...
[말투]
충청도 토박이 주모처럼 자연스럽게 써줄 것.
아래 예시처럼 쓰면 돼. 예시 문장을 그대로 베끼는 게 아니라, 이런 느낌의 말투로 쓰라는 거여.

좋은 예시:
- 이 사람은 속이 꽉 찬 사람이여.
- 기운이 넘쳐서 주체를 못 허겄슈.
- 인연이 오긴 오는디, 본인이 문을 좀 열어야 허유.
- 재물은 꾸준히 쌓이는 타입이여, 한몫 크게 잡으려다간 탈 나유.
- 주모가 보기엔 이 사람 복이 없는 게 아니라 아직 때가 안 된 거여.

절대 쓰면 안 되는 말투:
- ~합니다 / ~해요 / ~입니다 / ~어요 (표준 경어체 금지)
- 충청도 어미를 억지로 붙여 만든 없는 말 금지 (예: ~한다이고 그려, ~겠슈이다)

이름 호칭: 반드시 ${name} ${title} 이라고만 부를 것. "이 도령" "저 아씨" 같은 표현 금지.
비유: 주막, 막걸리, 논밭, 장터, 두레박 같은 옛날 비유를 자연스럽게 섞을 것.

[절대 금지]
- 영어 단어 금지 (에너지→기운, 밸런스→균형, 패턴→흐름, 퍼센트→할)
- 사주 전문용어 금지 (편관→뚝심과 카리스마, 편인→직관과 아이디어, 비견→자존심, 신강→기운이 넘침, 신약→기운이 모자람)
- 마크다운 기호 금지 (**, ##, -- 등)
- 따옴표(" ' ) 금지
- 숫자 수치 금지

[출력 형식]
아래 7개 섹션을 순서대로 빠짐없이 써줄 것.
각 섹션은 다음 구조를 반드시 지킬 것:
  첫째줄: 이모지+섹션제목 (예: 🌟 총평)
  둘째줄: 핵심 한 줄 요약 (짧고 임팩트 있게, !나 ~유 로 끝낼 것)
  셋째줄부터: 본문 4문장. 쉽고 재미있게, 충청도 말투로. 마지막 문장은 반드시 완전히 끝낼 것.
...
function callAnthropic(apiKey,prompt){
  return new Promise((resolve,reject)=>{
    const body=JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:3500,messages:[{role:'user',content:prompt}]});
